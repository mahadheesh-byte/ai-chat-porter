// You.com (you.com) - generic chat export/import
...
function extractMessages() {
  const messages = [];
  const containers = document.querySelectorAll('[class*="message"], [class*="Message"], [class*="chat"], [data-role], [class*="response"], article');
  if (containers.length > 0) {
    containers.forEach((container) => {
      ...
      const isUser = container.getAttribute('data-role') === 'user' || container.classList.contains('user') || container.classList.contains('human') ||
        (container.querySelector('[class*="user"], [class*="human"], [class*="query"]') !== null);
      messages.push({
  ...
  if (messages.length === 0) throw new Error('No messages found. Make sure you have an active You.com conversation.');
  return messages;
}
...
async function injectConversation(messages, condensedPrompt) {
  ...
  if (!inputBox) throw new Error('Could not find chat input. Make sure you\'re on the You.com chat page.');
  inputBox.focus();
