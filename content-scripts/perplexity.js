// Perplexity (perplexity.ai) - generic chat export/import
...
function extractMessages() {
  const messages = [];
  const containers = document.querySelectorAll('[class*="message"], [class*="Message"], [class*="chat"], [data-role], article');
  if (containers.length > 0) {
    containers.forEach((container) => {
      ...
      const isUser = container.getAttribute('data-role') === 'user' || container.classList.contains('user') || container.classList.contains('human') ||
        (container.querySelector('[class*="user"], [class*="human"]') !== null);
      messages.push({
  ...
  if (messages.length === 0) throw new Error('No messages found. Make sure you have an active Perplexity conversation.');
  return messages;
}
...
async function injectConversation(messages, condensedPrompt) {
  ...
  if (!inputBox) throw new Error('Could not find chat input. Make sure you\'re on the Perplexity chat page.');
  inputBox.focus();
