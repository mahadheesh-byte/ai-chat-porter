// Microsoft Copilot (copilot.microsoft.com, bing.com) - generic chat export/import
...
function extractMessages() {
  const messages = [];
  const containers = document.querySelectorAll('[class*="message"], [class*="Message"], [class*="chat"], [data-role], [class*="turn"], article');
  if (containers.length > 0) {
    containers.forEach((container) => {
      const content = (container.textContent || '').trim();
      const attachments = extractAttachmentsFromContainer(container);
      if (!content && attachments.length === 0) return;
      const isUser = container.getAttribute('data-role') === 'user' || container.classList.contains('user') || container.classList.contains('human') ||
        (container.querySelector('[class*="user"], [class*="human"], [class*="request"]') !== null);
      messages.push({
  ...
  if (messages.length === 0) {
    ...
    if (main) {
      const blocks = main.querySelectorAll('[class*="message"], [class*="bubble"], [class*="response"], p');
      let role = 'user';
      blocks.forEach((el) => {
  ...
  if (messages.length === 0) throw new Error('No messages found. Make sure you have an active Copilot conversation.');
  return messages;
}
...
async function injectConversation(messages, condensedPrompt) {
  ...
  if (!inputBox) throw new Error('Could not find chat input. Make sure you\'re on the Copilot chat page.');
  inputBox.focus();
