// Meta AI (meta.ai) - generic chat export/import
...
function extractMessages() {
  const messages = [];
  const containers = document.querySelectorAll('[class*="message"], [class*="Message"], [class*="chat"], [data-role], [class*="bubble"], article');
  ...
  if (messages.length === 0) throw new Error('No messages found. Make sure you have an active Meta AI conversation.');
  return messages;
}
...
async function injectConversation(messages, condensedPrompt) {
  ...
  if (!inputBox) throw new Error('Could not find chat input. Make sure you\'re on the Meta AI chat page.');
  inputBox.focus();
